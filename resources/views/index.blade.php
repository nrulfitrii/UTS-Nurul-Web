                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$bunga->harga}}</td>
                            <td>{{$bunga->stok}}</td>
                            <td><img src="{{asset('storage/'.$bunga->gambar)}}" alt="Rose" width="50"></td>
                            <td>
                                <a href="{{route('dashboard.edit', $bunga->id)}}">
                                    <button class="btn btn-primary btn-sm" data-toggle="modal"
                                        data-target="#editProductModal">Edit</button>
                                </a>
                                <form action="{{route('dashboard.destroy',$bunga->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
